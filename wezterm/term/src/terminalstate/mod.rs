...
impl ThreadedWriter {
    ...
    /// 阻塞等待后台线程把已入队的所有字节写到底层 writer
    fn sync(&self) {
        let (ack_tx, ack_rx) = channel::<()>();
        if self.sender.send(WriterMessage::Sync(ack_tx)).is_ok() {
            let _ = ack_rx.recv();
        }
    }
}
...
impl TerminalState {
    /// 同步刷新：把 BufWriter 缓冲刷到后台线程，并阻塞等待后台线程
    /// 把已入队的所有字节写到底层 writer。用于读取 writer 输出前
    /// （如取编码后的输入字节 / 应答序列）消除异步竞态。
    pub fn flush_sync(&mut self) {
        let _ = self.writer.flush();
        self.writer.get_ref().sync();
    }
