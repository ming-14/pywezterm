...
/// 侧载 conpty.dll 所在目录（wezterm 自带 OpenConsole 宿主）。
/// 由上层绑定（如 Python 扩展）在初始化时通过 [`set_conpty_dir`] 指定；
/// 未指定时回落系统 conhost（kernel32）。
#[cfg(windows)]
pub static CONPTY_DIR: std::sync::OnceLock<std::path::PathBuf> = std::sync::OnceLock::new();

/// 指定侧载 conpty.dll + OpenConsole.exe 的部署目录（二者须同目录）
#[cfg(windows)]
pub fn set_conpty_dir(dir: std::path::PathBuf) {
    let _ = CONPTY_DIR.set(dir);
}
...
pub trait MasterPty: Downcast + Send {
    ...
    /// If applicable, return the underlying ConPTY HPCON handle
    /// (Windows only), allowing external processes to attach to the
    /// same pseudo console via CreateProcess (e.g. sandbox spawn).
    #[cfg(windows)]
    fn hpcon(&self) -> Option<RawHandle> {
        None
    }
}
