...
// 字段声明顺序即 drop 顺序：readable（输出读句柄）必须先于 con 释放，
// 否则 ClosePseudoConsole 调用时读句柄仍打开，conhost 无法退出造成死锁
struct Inner {
...
impl MasterPty for ConPtyMasterPty {
    ...
    #[cfg(windows)]
    fn hpcon(&self) -> Option<std::os::windows::io::RawHandle> {
        Some(self.inner.lock().unwrap().con.hpcon() as _)
    }
}
